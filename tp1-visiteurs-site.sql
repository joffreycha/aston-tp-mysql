...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nombre_de_visiteurs` AS select count(`visiteur`.`nombre_visites`) AS `nombre_de_visiteurs` from `visiteur`;
