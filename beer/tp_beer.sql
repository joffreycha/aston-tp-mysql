...
(
	...
	FROM ventes
	WHERE ANNEE = 2015
	AND ventes.ID_ARTICLE = article.ID_ARTICLE
) AS QUANTITE_VENDUE_2015,
( 
	SELECT SUM(QUANTITE) 
	FROM ventes
	WHERE ANNEE = 2016
	AND ventes.ID_ARTICLE = article.ID_ARTICLE
) AS QUANTITE_VENDUE_2016
FROM article
INNER JOIN ventes using(ID_ARTICLE)
GROUP BY ID_ARTICLE
HAVING ABS(((QUANTITE_VENDUE_2016 - QUANTITE_VENDUE_2015) / QUANTITE_VENDUE_2016 * 100)) < 1;
...
SELECT NOM_TYPE, 
(	# ventes 2016 - ventes 2015 = evolution des ventes
	(
		SELECT SUM(QUANTITE)
		FROM ventes
		INNER JOIN article using(ID_ARTICLE)
